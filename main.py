...
class OTPMonitorBot:
    ...
    def __init__(self, telegram_token, group_chat_id, session_cookie, target_url, target_host):
        ...
        self.processed_count = 0  # memory ржирж┐ржпрж╝ржирзНрждрзНрж░ржгрзЗрж░ ржЬржирзНржп
        ...
        # OTP ржкрзНржпрж╛ржЯрж╛рж░рзНржи тАФ specific ржЖржЧрзЗ, generic ржкрж░рзЗ

        self.otp_patterns = [

            r'#(\d{3}\s\d{3})',                # #209 658 (Instagram)

            r'(?<!\d)(\d{3})\s(\d{3})(?!\d)',  # 209 658 рж╕рзНржкрзЗрж╕ ржжрж┐ржпрж╝рзЗ

            r'(?<!\d)(\d{3})-(\d{3})(?!\d)',   # 209-658 dash ржжрж┐ржпрж╝рзЗ

            r'code[:\s]+(\d{4,8})',             # code: 123456

            r'ржХрзЛржб[:\s]+(\d{4,8})',              # ржмрж╛ржВрж▓рж╛ ржХрзЛржб

            r'(?<!\d)(\d{6})(?!\d)',            # 6 ржбрж┐ржЬрж┐ржЯ

            r'(?<!\d)(\d{5})(?!\d)',            # 5 ржбрж┐ржЬрж┐ржЯ

            r'(?<!\d)(\d{4})(?!\d)',            # 4 ржбрж┐ржЬрж┐ржЯ
    ...
    def escape_markdown(self, text):
        ...
        """Markdown special character escape ржХрж░рзЛ ржпрж╛рждрзЗ Telegram error ржирж╛ ржжрзЗржпрж╝"""

        text = str(text)

        # backtick-ржПрж░ ржоржзрзНржпрзЗ ржкрж╛ржарж╛ржм рждрж╛ржЗ рж╢рзБржзрзБ backtick escape ржХрж░рж▓рзЗржЗ рж╣ржмрзЗ
    ...
    async def send_telegram_message(self, message, chat_id=None, reply_markup=None):
        ...
        try:
            ...
            logger.info(f"тЭМ Telegram Error: {e}")

            print(f"тЭМ Telegram Error: {e}")

            return False

        except Exception as e:

            logger.info(f"тЭМ Send Message Error: {e}")

            print(f"тЭМ Send Message Error: {e}")
    ...
    async def send_startup_message(self):
        ...
        startup_msg = (
            ...
            "ЁЯЪА *ржУржЯрж┐ржкрж┐ ржоржирж┐ржЯрж░ ржмржЯ рж╕рзНржЯрж╛рж░рзНржЯ рж╣ржпрж╝рзЗржЫрзЗ* ЁЯЪА\n\n"

            "тХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХР\n\n"

            "тЬЕ *рж╕рзНржЯрзЗржЯрж╛рж╕:* `рж▓рж╛ржЗржн & ржоржирж┐ржЯрж░рж┐ржВ`\n"

            "тЪб *ржорзЛржб:* `ржлрж╛рж░рзНрж╕рзНржЯ ржУржЯрж┐ржкрж┐ ржЕржирж▓рж┐`\n"

            f"ЁЯУб *рж╣рзЛрж╕рзНржЯ:* `{self.target_host}`\n\n"

            f"тП░ *рж╕рзНржЯрж╛рж░рзНржЯ ржЯрж╛ржЗржо:* `{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`\n\n"

            "тХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХР\n"

            "ЁЯдЦ *ржУржЯрж┐ржкрж┐ ржоржирж┐ржЯрж░ ржмржЯ ржЪрж▓ржЫрзЗ*"
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("ЁЯСитАНЁЯТ╗ Developer", url="https://t.me/FBDEALZONEOWNER")],

            [InlineKeyboardButton("ЁЯУв Channel", url="https://t.me/FBDEALZONEofficial")]
        ...
        try:
            ...
            if success:
                ...
                logger.info("тЬЕ Startup message sent to group")

        except Exception as e:

            logger.info(f"тЪая╕П Startup message failed (monitoring will continue): {e}")



    def extract_otp(self, message):

        """ржорзЗрж╕рзЗржЬ ржерзЗржХрзЗ OTP ржПржХрзНрж╕ржЯрзНрж░рж╛ржХрзНржЯ тАФ timestamp/date ржмрж╛ржж ржжрж┐ржпрж╝рзЗ"""
    ...
    def format_message(self, sms_data, message_text, otp_code):
        ...
        return (
            ...
            "ЁЯФе *ржирждрзБржи ржУржЯрж┐ржкрж┐ ржбрж┐ржЯрзЗржХрзНржЯрзЗржб* ЁЯФе\n"

            "тХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХР\n\n"

            f"ЁЯУЕ *ржЯрж╛ржЗржо:* `{timestamp}`\n"

            f"ЁЯУ▒ *ржиржорзНржмрж░:* `{phone}`\n"

            f"ЁЯПв *ржЕржкрж╛рж░рзЗржЯрж░:* `{operator}`\n"

            f"ЁЯМР *рж╕рж╛рж░рзНржнрж┐рж╕:* `{service}`\n\n"

            f"ЁЯОп *ржУржЯрж┐ржкрж┐ ржХрзЛржб:* `{code}`\n\n"

            f"ЁЯУЭ *ржорзЗрж╕рзЗржЬ:*\n`{msg}`\n\n"

            "тХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХР\n"

            "ЁЯдЦ *ржУржЯрж┐ржкрж┐ ржоржирж┐ржЯрж░ ржмржЯ*"
    ...
    def create_response_buttons(self):
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("ЁЯУ▒ Number Channel", url="https://t.me/FBDEALZONEofficial")],
    ...
    async def monitor_loop(self):
        ...
        logger.info("ЁЯЪА OTP Monitoring Started - FIRST OTP ONLY")
        ...
        while self.is_monitoring:
            ...
            try:
                ...
                logger.info(f"ЁЯФН Check #{check_count} at {current_time}")
                ...
                if data and 'aaData' in data:
                    ...
                    if valid_sms:
                        ...
                        # index 0,1,2,3 skip тАФ рж╢рзБржзрзБ SMS body field ржерзЗржХрзЗ OTP ржЦрзЛржБржЬрзЛ
                        ...
                        for i, field in enumerate(first_sms):
                            ...
                            if isinstance(field, str) and len(field) > 3 and field.strip() not in ('$', '', '-'):
                                ...
                                if found:
                                    ...
                                    logger.info(f"ЁЯУН OTP found at index {i}: {field[:80]}")
                        ...
                        if otp_id not in self.processed_otps:
                            ...
                            logger.info(f"ЁЯЪи FIRST OTP DETECTED: {timestamp}")



                            if otp_code:

                                logger.info(f"ЁЯФР OTP Code: {otp_code}")
                                ...
                                # success ржмрж╛ fail ржпрж╛ржЗ рж╣рзЛржХ processed-ржП add ржХрж░рзЛ
                                ...
                                if self.processed_count >= 1000:
                                    ...
                                    logger.info("ЁЯз╣ Processed OTPs cache cleared")
                                ...
                                if success:
                                    ...
                                    logger.info(f"тЬЕ OTP SENT: {timestamp} - Total: {self.total_otps_sent}")

                                else:

                                    logger.info(f"тЭМ Telegram send failed: {timestamp}")

                            else:

                                self.processed_otps.add(otp_id)

                                logger.info(f"тЪая╕П OTP not found. Full data: {first_sms}")

                        else:

                            logger.debug(f"тПй Already Processed: {timestamp}")

                    else:

                        logger.info("тД╣я╕П No valid SMS records found")

                else:

                    logger.warning("тЪая╕П No data from API")



                if check_count % 20 == 0:

                    logger.info(f"ЁЯУК Status - Total OTPs Sent: {self.total_otps_sent}")
                ...
                logger.error(f"тЭМ Monitor Loop Error: {e}")

                print(f"тЭМ Monitor Loop Error: {e}")
...
async def main():
    ...
    SESSION_COOKIE = "1koo0dblehit1kk8hcpse6vlcd"
    ...
    print("ЁЯдЦ OTP MONITOR BOT - FIRST OTP ONLY")

    print("=" * 50)

    print(f"ЁЯУб Host: {TARGET_HOST}")

    print("ЁЯУ▒ Group ID:", GROUP_CHAT_ID)
    ...
    try:
        ...
        print(f"ЁЯУК Total OTPs Sent: {otp_bot.total_otps_sent}")
